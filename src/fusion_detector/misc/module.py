...
class ModuleProxy(AbstractModuleProxy):
    ...
    def __init__(
        ...
        *,
        ...
        self.criteria: Optional[CriteriaAction] = None
    ...
    def train(self, data: DataLoader, epochs: int) -> float:
        self.module.train()
        final_loss = 0
        with tqdm(total=epochs, desc="Epochs") as progress:
            for _ in range(epochs):
                ...
                for x, label in tqdm(data, desc="Dataset"):
                    ...
                    # Backward and step.
                    loss: torch.Tensor = self.criteria(y, label)
                    loss.backward()
                    self.optimizer.step()
                    # Collect statistics
                ...
                progress.update()
                # Passes the epoch's loss to final loss.
    ...
    def evaluate(self, x: torch.Tensor) -> torch.Tensor:
        self.module.eval()
        return self.module(self.preprocess_input(apply_device(x)))
    ...
    def test(self, data: DataLoader) -> float:
        ...
        correct = 0
        total = 0
        with tqdm(total=len(data), desc="Test (Dataset)") as progress:
            for x, label in data:
                # Apply device
                x, label = apply_device(x, label)
                # Preprocess input and label
...
class MultiModuleProxy(AbstractModuleProxy):
    ...
    def __init__(
        ...
        *modules: Union[nn.Module, ModuleProxy],
        ...
        self.criteria: Optional[ListOrElementProxy[CriteriaAction]] = None
        if criteria is not None:
            self.criteria = ListOrElementProxy(criteria)

    def train(self, data: DataLoader, epochs: int) -> List[float]:
        """Train all managed modules on the same dataset."""
        losses = []
        for proxy in tqdm(self.proxies, desc="Training Models"):
            loss = proxy.train(data, epochs)
            losses.append(loss)
        return losses
    ...
    def test(self, data: DataLoader) -> List[float]:
        accuracies = []
        for proxy in tqdm(self.proxies, desc="Testing Models"):
            accuracy = proxy.test(data)
...
class AdversarialModuleProxy(ModuleProxy):
    ...
    def preprocess_input(self, x: Tensor) -> Tensor:
        # Generate adversarial examples and append them to the input tensor.
        ...
        self._shuffle = torch.randperm(inputs.size(0))
        return inputs[self._shuffle, ...]
